...
function prx($arr){
    ...
    function apply_coupon_code($coupon_code){
        ...
        if(isset($result[0])){
            ...
            if($result[0]->status==1){
                if($result[0]->is_one_time==1){
                    $status = "error";
                    $msg = "Coupon Code has already been used";
                }else{
        ...
        if($status=='success'){
            ...
            if($type=='Per'){
                $new_Price = ($value/100) * $totalPrice;
                $totalPrice = round($totalPrice - $new_Price);
                $coupon_code_value=$new_Price;
            }
        }

        return json_encode(['status'=>$status, 'msg'=>$msg, 'totalPrice'=>$totalPrice, 'value'=>$coupon_code_value, 'coupon_code_value'=> $coupon_code_value]);
    }

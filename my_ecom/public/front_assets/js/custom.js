...
function home_add_to_cart(id, size_skip_id, color_skip_id)
{
  jQuery('#size_id').val(size_skip_id);
  jQuery('#color_id').val(color_skip_id);
  add_to_cart(id, size_skip_id, color_skip_id);
}

function add_to_cart(id, size_skip_id, color_skip_id)
{
  ...
  if(size_skip_id == 0 && color_skip_id == 0)
  {
    size_id = 'skip';
    color_id = 'skip';
  }

  if(size_id == '' && size_id != 'skip'){
    jQuery('#add_to_cart_msg').html("<div class='alert alert-danger fade-in alert-dismissable mt-10'><a href='#' class='close' data-dismiss='alert' title='close'>x</a><span>Please Select Size</span></div>");
  }else if(color_id == '' && color_id != 'skip')
  {
    jQuery('#add_to_cart_msg').html("<div class='alert alert-danger fade-in alert-dismissable mt-10'><a href='#' class='close' data-dismiss='alert' title='close'>x</a><span>Please Select Color</span></div>");
  }else{
    jQuery('#product_id').val(id);
    jQuery('#product_qty').val(jQuery('#qty').val());
    jQuery.ajax({
      url: '/add_to_cart',
      ...
      success: function(result){
        ...
        if(result.totalItem==0){
          ...
          jQuery('.aa-cartbox-notify').html(result.totalItem);
          var html = '<ul>';
          jQuery.each(result.data, function(arrKey, arrVal){
              totalPrice = parseInt(totalPrice) + (parseInt(arrVal.qty)*parseInt(arrVal.price)); 
              html += '<li><a class="aa-cartbox-img" href="#"><img src="'+PRODUCT_IMAGE+'/'+arrVal.image+'"></a><div class="aa-cartbox-info"><h4><a href="#">'+arrVal.name+'</a></h4><p>$ '+arrVal.qty+' x '+arrVal.price+'</p></div></li>';
              jQuery(result.data).find('image').attr('src'); 
              console.log(arrVal);
            });
...
function updateQty(pid, size, color, attr_id, price)
{
  ...
  jQuery('#total_price_'+pid).html('$ '+qty*price);
}

function deleteCartProduct(pid, size, color, attr_id, price)
{
  jQuery('#size_id').val(size);
  jQuery('#color_id').val(color);
  jQuery('#qty').val(0);
  ...
  if(coupon_code!=''){
    jQuery.ajax({
      ...
      success: function(result){
        ...
        if(result.status=='success'){
          jQuery('.coupon_show_box').removeClass('hide');
          jQuery('#coupon_code_str').html(coupon_code);
          jQuery('#coupon_code_value').html(result.value);
  ...
  if(coupon_code!=''){
    jQuery.ajax({
      ...
      success: function(result){
        // console.log(result.status);
        if(result.status=='success'){
          jQuery('.coupon_show_box').addClass('hide');
